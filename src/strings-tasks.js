...
function formatTime(minutes, seconds) {
  return [minutes, seconds]
    .map((element) =>
      element < 10 ? element.toString().padStart(2, '0') : element.toString()
    )
    .join(':');
}
 ...
 *
 * @param {string} str - The input string.e
...
function orderAlphabetically(str) {
  return str
    .split('')
    .sort((prev, curr) => prev.charCodeAt(0) - curr.charCodeAt(0))
    .join('');
}
...
function findLongestWord(sentence) {
  ...
  const maxLength = arr
    ...
    .slice(-1)
    .join('').length;
  return arr.find((e) => e.length === maxLength);
}
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (letter) => newLetters[letters.indexOf(letter)]
  );
}
